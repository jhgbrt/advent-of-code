...
static class CommandHelper
{
    public static RootCommand CreateRootCommand()
    {
        var root = new RootCommand
        {
            ...
            CreateCommand<InitPuzzle.Options>("init", "initialize the code for a specific puzzle. Requires AOC_SESSION set as an environment variable", InitPuzzle),
            CreateCommand<SyncPuzzle.Options>("sync", "sync the data (specifically the posted answers) for a puzzle. Requires AOC_SESSION set as an environment variable", SyncPuzzle),
            CreateCommand<PostSolution.Options>("post", "post an answer for a puzzle part. Requires AOC_SESSION set as an environment variable", PostSolution),
            ...
            CreateCommand<ShowLeaderboard.Options>("leaderboard", "show some stats from the configured private leaderboard. Set AOC_LEADERBOARD_ID as a environment variable.", ShowLeaderboard),
        };
        return root;
    }
    ...
    static async Task ShowLeaderboard(ShowLeaderboard.Options options) => await new ShowLeaderboard(Factory.CreateClient()).Run(options);
