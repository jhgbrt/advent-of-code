...
public class AoC202125
{
    static string[] input = Read.InputLines();
    public object Part1() => FindLastMove(new Grid(
...
record Grid(ImmutableDictionary<P,char> items, int Height, int Width)
{
    public IEnumerable<P> ValidMoves(char direction) => (
            from item in items
            where item.Value == direction
            && !items.ContainsKey(item.Key.Next(Width, Height, direction))
            select item.Key
            );
