...
public class AoC202218
{
    ...
    static IEnumerable<Coordinate> Coordinates = (
        ...
        select new Coordinate(int.Parse(split[0]), int.Parse(split[1]), int.Parse(split[2]))
        ).ToImmutableArray();

    static readonly int maxX = Coordinates.Max(c => c.x);
    static readonly int maxY = Coordinates.Max(c => c.y);
    static readonly int maxZ = Coordinates.Max(c => c.z);
    static Coordinate origin = new Coordinate(0, 0, 0);
    static Coordinate max = new Coordinate(maxX, maxY, maxZ);

    public int Part1() => SurfaceArea(Coordinates);
    public int Part2() => SurfaceArea(Fill(Coordinates));

    private int SurfaceArea(IEnumerable<Coordinate> set)
    {
        var x = from c in set
    ...
    IEnumerable<Coordinate> Fill(IEnumerable<Coordinate> coordinates)
    {
        ...
        var all = from x in Range(0, maxX)
    ...
    public void Draw(HashSet<Coordinate> set)
    {
        int z = 0;
        while (true)
        {
    ...
    void Draw(HashSet<Coordinate> set, int z)
    {
    ...
    bool IsContained(Coordinate c, HashSet<Coordinate> set)
    {
        ...
        if (c == new Coordinate(3, 9, 12)) Debugger.Break();
        while (queue.Count > 0)
        {
