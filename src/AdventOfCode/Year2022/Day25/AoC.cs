...
public class AoC202225
{
    static string[] input = Read.InputLines();

    public Snafu Part1() => input.Select(Snafu.Parse).Aggregate(Snafu.Zero, (l, r) => l + r);

    public Snafu Part2() => Snafu.Zero;
}
...
public readonly record struct Snafu(ImmutableArray<int> digits)
{
    ...
    public int ToInt32() => (
        from t in digits.Select((digit, p) => (digit, p))
        select t.digit * (int)Pow(5, t.p)
        ).Sum();
    ...
    public override string ToString() => digits
        .Reverse()
        .Aggregate(new StringBuilder(), (sb, d) => sb.Append(d switch
        {
            -2 => '=',
            -1 => '-',
            _ => (char) (d + '0')
    ...
    public static Snafu operator+(Snafu left, Snafu right)
    {
        ...
        while (result.Last() == 0) result.RemoveAt(result.Count - 1);
