...
public class AoC202216
{
    static Regex regex = new(@"Valve (?<src>[A-Z]+) has flow rate=(?<rate>\d+); tunnel[s]? lead[s]? to valves? (?<dest>[A-Z ,]*)");

    static string[] sample = Read.SampleLines();
    static string[] input = Read.InputLines();

    static Valve[] valves = (from line in input
    ...
    static ImmutableArray<Valve> nonzeroValves = (from v in valves
                                                  where v.rate > 0
                                                  orderby v.rate descending
                                                  select v).ToImmutableArray();
    static IImmutableDictionary<(string, string), int> ShortestPaths = GetShortestPaths();
    static IImmutableDictionary<(string, string), int> GetShortestPaths()
    {
        var graph = (
            from valve in valves
            from destination in valve.destinations
            select new Edge<string>(valve.name, destination)
    ...
    public object Part1() => FindPath(30, "AA", 0, ImmutableHashSet<string>.Empty, nonzeroValves, ShortestPaths);

    public int Part2() => FindPath2(0, "AA", nonzeroValves, valves.ToImmutableDictionary(v => v.name), (26, 0), false);
