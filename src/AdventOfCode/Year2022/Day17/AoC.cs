...
public class AoC202217
{
    static string[] sample = Read.SampleLines();
    static string[] input = Read.InputLines();

    public object Part1() => new Tetris(input[0]).Play1(2022);
    public object Part2() => new Tetris(input[0]).Play2(1000000000000); //PartTwo(sample[0]); //
}
...
readonly record struct Rock(Point origin, ImmutableHashSet<Point> shape, long height, int width)
{
    ...
    {
        return from py in items.Select((l, i) => (l, i))
    ...
    public bool Overlaps(HashSet<Point> other) 
        => other.Overlaps(All());
...
class Tetris
{
    ImmutableArray<Rock> rocksarray = new[]
    {
            new[]
            {
                "####"
    ...
    private Rock GetRock(long i) => rocksarray[(int)(i % 5)];
