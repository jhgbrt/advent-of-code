...
public class AoC202219
{
    static readonly Regex regex = new Regex(
        @"Blueprint (?<id>\d+): Each ore robot costs (?<ore>\d+) ore. Each clay robot costs (?<clay_ore>\d+) ore. Each obsidian robot costs (?<obsidian_ore>\d+) ore and (?<obsidian_clay>\d+) clay. Each geode robot costs (?<geode_ore>\d+) ore and (?<geode_obsidian>\d+) obsidian.",
        RegexOptions.Compiled);

    static string[] input = Read.InputLines();

    static readonly ImmutableArray<Blueprint> blueprints = (
        from line in input
        ...
        select new Blueprint(
            ...
            obsidian: (ore: int.Parse(match.Groups["obsidian_ore"].Value), clay: int.Parse(match.Groups["obsidian_clay"].Value)),
            geode: (ore: int.Parse(match.Groups["geode_ore"].Value), obsidian: int.Parse(match.Groups["geode_obsidian"].Value)
            ))).ToImmutableArray();

    public object Part1() => (from b in blueprints
                              select b.id * UseBlueprint(b)).Sum();

    public object Part2() => (from b in blueprints.Take(3)
                              select UseBlueprint(b, 32)).Aggregate(1, (l, r) => l * r);

    private int UseBlueprint(Blueprint blueprint, int startTime = 24)
    {
        ...
        while (queue.TryDequeue(out var item))
        {
            ...
            if (cache.Contains((state, time))) continue;
...
record struct Blueprint(int id, int ore, (int ore, int _) clay, (int ore, int clay) obsidian, (int ore, int obsidian) geode)
{
    ...
    public (Resources resources, Robots robots) BuyOreRobot(Resources resources, Robots robots) => (
            resources with { ore = resources.ore - ore },
            robots with { ore = robots.ore + 1 }
            );
    public (Resources resources, Robots robots) BuyClayRobot(Resources resources, Robots robots) => (
            resources with { ore = resources.ore - clay.ore },
            robots with { clay = robots.clay + 1 }
            );
    public (Resources resources, Robots robots) BuyObsidianRobot(Resources resources, Robots robots) => (
            resources with { ore = resources.ore - obsidian.ore, clay = resources.clay - obsidian.clay },
            robots with { obsidian = robots.obsidian + 1 }
            );
    public (Resources resources, Robots robots) BuyGeodeRobot(Resources resources, Robots robots) => (
            resources with { ore = resources.ore - geode.ore, obsidian = resources.obsidian - geode.obsidian },
...
{
    public Resources Harvest(Resources resources) => resources with
    {
        ore = resources.ore + ore,
        clay = resources.clay + clay,
        obsidian = resources.obsidian + obsidian,
        geode = resources.geode + geode
    };
