...
public class AoC202212
{
    static string[] input = Read.InputLines();
    public int Part1() => Grid.Parse(input).CountSteps();

    public int Part2() => Grid.Parse(input).FewestSteps();
}
class Grid
{
    int[,] _grid;
    ...
    private AdjacencyGraph<(int, int), Edge<(int, int)>> Graph() => (
            ...
            select new Edge<(int x, int y)>(coordinate, n)
            ).ToAdjacencyGraph<(int, int), Edge<(int, int)>>();

    public int CountSteps() => 
        CountSteps(Graph(), Start);
    ...
    public override string ToString()
    {
        ...
        for (int y = 0; y < Height; y++)
        {
            for (int x = 0; x < Width; x++) sb.Append((char)(this[x, y] + 'a'));
            sb.AppendLine();
        }
        return sb.ToString();
    }
