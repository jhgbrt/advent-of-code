...
public class AoC202208
{
    static string[] input = Read.InputLines();
    static Grid grid = new Grid(input);
    public object Part1() => (from p in grid.InteriorPoints()
                              where grid.IsVisible(p)
                              select p).Count() + grid.Circumference;
    public object Part2() => (from p in grid.InteriorPoints()
                              select grid.Score(p)).Max();
}
...
class Grid
{
    ...
    internal bool IsVisible(Point p) => 
           Range(0, p.x).All(x => this[new(x, p.y)] < this[p])
        || Range(p.x + 1, Width - p.x - 1).All(x => this[new(x, p.y)] < this[p])
        || Range(0, p.y).All(y => this[new(p.x, y)] < this[p])
        || Range(p.y + 1, Height - p.y - 1).All(y => this[new(p.x, y)] < this[p]);
    ...
    {
        for (int x = p.x - 1; x >= 0; x--) yield return p with { x = x };
    }
    IEnumerable<Point> Right(Point p)
    {
        for (int x = p.x + 1; x < Width; x++) yield return p with { x = x};
    }
    IEnumerable<Point> Down(Point p)
    {
        for (int y = p.y + 1; y < Height; y++) yield return p with { y = y };
    }
    IEnumerable<Point> Up(Point p)
    {
        for (int y = p.y - 1; y >= 0; y--) yield return p with { y = y };
    }
}
