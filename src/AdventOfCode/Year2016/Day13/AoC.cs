...
public class AoC201613
{
    ...
    public object Part1()
    {
        ...
        while (true)
        {
            ...
            foreach (var next in prev.Neighbours()
                .Where(n => IsSpace(n.x, n.y) && !visited.Contains(n)))
            {
...
class Grid
{
    ...
    public char this[uint x, uint y]
    {
        get
        {
            if (map.TryGetValue((x, y), out char value)) return value;
            var v = IsSpace(x, y) ? '.' : '#';
            map[(x, y)] = v;
            return v;
        }
    ...
    private AdjacencyGraph<(uint, uint), Edge<(uint, uint)>> Graph() => (
           ...
           where this[n.x, n.y] != '#'
           select new Edge<(uint x, uint y)>(coordinate, n)
           ).ToAdjacencyGraph<(uint, uint), Edge<(uint, uint)>>();
