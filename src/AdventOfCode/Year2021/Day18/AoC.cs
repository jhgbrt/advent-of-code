...
public class AoC202118
{
    static string[] input = Read.InputLines();
    static ImmutableList<SnailFish> snailfish = input.Select(SnailFish.Parse).ToImmutableList();
    public object Part1() => snailfish.Aggregate((result, n) => (result + n).Reduce()).Magnitude();
    public object Part2() => (from s1 in snailfish
...
record SnailFish(ImmutableList<Token> Items)
{
    ...
    static IEnumerable<Token> Elements(string line) => from c in line
                                                         where c != ','
                                                         select c switch
                                                         {
                                                             '[' => Token.OpenBrace,
    ...
    public static SnailFish operator +(SnailFish a, SnailFish b) => new SnailFish(
            Empty<Token>()
    ...
    static (int? left, int? right) FindLeftAndRightValues(ImmutableList<Token>.Builder builder, int position)
        => (left: (from i in Range(0, position)
                   where builder[position - i].IsValue
                   select (int?)(position - i)).FirstOrDefault(),
            right: (from i in Range(position + 4, builder.Count - position - 4)
                    where builder[i].IsValue
                    select (int?)i).FirstOrDefault());
