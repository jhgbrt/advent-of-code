namespace AdventOfCode.Template.CSharp;
...
public partial class AoC
{
    ...
    readonly Answer answer = JsonSerializer.Deserialize<Answer>(File.ReadAllText("answers.json"))!;
    [Fact]
    public void TestPart1()
    {
        if (answer.part1 is not null)
        {
            var result = Part1().Value;
            if (result is null) throw new Exception("Puzzle 1 has an answer but no code");
            Assert.Equal(answer.part1.ToString(), Part1().Value.ToString());
        }
        else
        {
    ...
    public void TestPart2()
    {
        ...
        if (answer.part2 is not null)
        {
            var result = Part1().Value;
            if (result is null) throw new Exception("Puzzle 1 has an answer but no code");
            Assert.Equal(answer.part2.ToString(), Part2().Value.ToString());
        }
        else
        {
