...
static class AoC
{
    ...
    public static Result<int> Part1() => Run(1, () => input.Select(s => new IPAddress(s)).Where(ip => ip.SupportsTLS()).Count());
    public static Result<int> Part2() => Run(2, () => input.Select(s => new IPAddress(s)).Where(ip => ip.SupportsSSL()).Count());
