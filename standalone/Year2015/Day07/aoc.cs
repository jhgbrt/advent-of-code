...
Dictionary<string, Node> BuildNodes()
{
    var lines = input;
    var regexes = new (Regex r, Func<Match, Node> f)[] { (new(@"^(?<value>[\d]+) -> (?<name>[a-z]+)$"), m => new LiteralValueNode(m.Groups["name"].Value, int.Parse(m.Groups["value"].Value))), (new(@"^(?<value>[a-z]+) -> (?<name>[a-z]+)$"), m => new ConnectorNode(m.Groups["name"].Value, m.Groups["value"].Value)), (new(@"^NOT (?<value>[a-z]+) -> (?<name>[a-z]+)$"), m => new NotNode(m.Groups["name"].Value, m.Groups["value"].Value)), (new(@"^(?<left>[a-z]+) AND (?<right>[a-z]+) -> (?<name>[a-z]+)$"), m => new AndNode(m.Groups["name"].Value, m.Groups["left"].Value, m.Groups["right"].Value)), (new(@"^(?<left>[\d]+) AND (?<right>[a-z]+) -> (?<name>[a-z]+)$"), m => new AndValueNode(m.Groups["name"].Value, int.Parse(m.Groups["left"].Value), m.Groups["right"].Value)), (new(@"^(?<left>[a-z]+) OR (?<right>[a-z]+) -> (?<name>[a-z]+)$"), m => new OrNode(m.Groups["name"].Value, m.Groups["left"].Value, m.Groups["right"].Value)), (new(@"^(?<operand>[a-z]+) LSHIFT (?<value>\d+) -> (?<name>[a-z]+)$"), m => new LeftShiftNode(m.Groups["name"].Value, m.Groups["operand"].Value, int.Parse(m.Groups["value"].Value))), (new(@"^(?<operand>[a-z]+) RSHIFT (?<value>\d+) -> (?<name>[a-z]+)$"), m => new RightShiftNode(m.Groups["name"].Value, m.Groups["operand"].Value, int.Parse(m.Groups["value"].Value))) };
...
void Resolve(Dictionary<string, Node> nodes)
{
...
record AndNode(string Name, string Left, string Right) : Node(Name)
{
    ...
    public override void Resolve(IReadOnlyDictionary<string, Node> nodes)
    {
        LeftOperand = nodes[Left];
        RightOperand = nodes[Right];
    }
...
record AndValueNode(string Name, int Value, string RightOperandName) : Node(Name)
{
    ...
    public override void Resolve(IReadOnlyDictionary<string, Node> nodes)
    {
        RightOperand = nodes[RightOperandName];
    }
...
record OrNode(string Name, string LeftOperandName, string RightOperandName) : Node(Name)
{
    ...
    public override void Resolve(IReadOnlyDictionary<string, Node> nodes)
    {
        LeftOperand = nodes[LeftOperandName];
        RightOperand = nodes[RightOperandName];
    }
