...
var part1 = Run((grid, c) => grid[c.x, c.y] = 1, (grid, c) => grid[c.x, c.y] = 0, (grid, c) => grid[c.x, c.y] = grid[c.x, c.y] == 0 ? 1 : 0);
var part2 = Run((grid, c) => grid[c.x, c.y] += 1, (grid, c) => grid[c.x, c.y] = Math.Max(0, grid[c.x, c.y] - 1), (grid, c) => grid[c.x, c.y] = grid[c.x, c.y] += 2);
Console.WriteLine((part1, part2, sw.Elapsed));
object Run(Action<int[,], Coordinate> turnon, Action<int[,], Coordinate> turnoff, Action<int[,], Coordinate> toggle)
{
...
void ApplyInstruction(int[,] grid, Instruction instruction, Action<int[,], Coordinate> turnon, Action<int[,], Coordinate> turnoff, Action<int[,], Coordinate> toggle)
{
