...
var layers = input.SelectMany(c => c).Select(c => c - '0').Chunk(25 * 6);
var sw = Stopwatch.StartNew();
var part1 = (
    from layer in layers
    select (layer, count: layer.Count(x => x == 0))).MinBy(x => x.count).layer.Where(i => i is 1 or 2).Aggregate((ones: 0, twos: 0), (p, i) => i == 1 ? (p.ones + 1, p.twos) : (p.ones, p.twos + 1)).Product();
var part2 = Part2();
...
object Part2()
{
    ...
    var q =
        from x in result.Chunk(25).Select((line, n) => (line, n))
        let line = x.line
        let n = x.n
