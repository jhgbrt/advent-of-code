...
var scanners = Align(
    from s in CreateScanners(input) from t in s.Transformations() select t);
...
{
    var remaining = (
        from scanner in scanners.Skip(1) group scanner by scanner.id).ToImmutableDictionary(g => g.Key);
