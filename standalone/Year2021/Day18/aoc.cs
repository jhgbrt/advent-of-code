...
var part2 = (
    ...
    where s1 != s2
    select (s1 + s2).Reduce().Magnitude()).DefaultIfEmpty().Max();
...
record SnailFish(ImmutableList<Token> Items)
{
    ...
    public static SnailFish operator +(SnailFish a, SnailFish b) => new SnailFish(Empty<Token>().Concat(Repeat(Token.OpenBrace, 1)).Concat(a.Items).Concat(b.Items).Concat(Repeat(Token.CloseBrace, 1)).ToImmutableList());
    ...
    static (int? left, int? right) FindLeftAndRightValues(ImmutableList<Token>.Builder builder, int position) => (left: (
        from i in Range(0, position)
        where builder[position - i].IsValue
        select (int?)(position - i)).FirstOrDefault(), right: (
