...
class Grid
{
    ...
    private Set FoldUp(Set coordinates, int v) => (
        ...
        select (d, x)).Aggregate(coordinates, (c, t) => c.Remove(new(t.x, v + t.d)).Add(new(t.x, v - t.d)));
    private Set FoldLeft(Set coordinates, int v) => (
        ...
        select (d, y)).Aggregate(coordinates, (c, t) => c.Remove(new(v + t.d, t.y)).Add(new(v - t.d, t.y)));
...
static class PixelFontDecoder
{
    public static string DecodePixels(this string s, int size, char pixel = '#', char blank = '.') => (
        ...
        select (
            ...
            select (char?)item.c).SingleOrDefault() ?? '?').Aggregate(new StringBuilder(), (sb, c) => sb.Append(c)).ToString();
    ...
    private static IEnumerable<Range> Lines(this string s)
    {
        ...
        while (x < s.Length)
        {
            ...
            if (newline == -1)
                break;
            var count = newline switch
            {
                > 0 when s[newline - 1] == '\r' => newline - x - 1,
                _ => newline - x
            };
    ...
    static readonly (string s, char c)[] letters = new[] { ".##.#..##..######..##..#", "###.#..####.#..##..####.", ".####...#...#...#....###", "###.#..##..##..##..####.", "#####...###.#...#...####", "#####...###.#...#...#...", ".####...#...#.###..#.##.", "#..##..######..##..##..#", "###..#...#...#...#..###.", "..##...#...#...##..#.##.", "#..##.#.##..##..#.#.#..#", "#...#...#...#...#...####", "#..######..##..##..##..#", "#..###.###.##.###.###..#", ".##.#..##..##..##..#.##.", "###.#..##..####.#...#...", ".##.#..##..##..##.##.###", "###.#..##..####.#.#.#..#", ".####...#....##....####.", "####.#...#...#...#...#..", "#..##..##..##..##..#.##.", "#..##..##..#.##..##..##.", "#..##..##..##..######..#", "#..##..#.##.#..##..##..#", "#..##..#.##...#...#..#..", "####...#..#..#..#...####" }.Select((s, i) => (s, (char)(i + 'A'))).ToArray();
}