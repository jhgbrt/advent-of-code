...
var recipe = input[0];
var transformations = (
    from line in input.Skip(2) let split = line.Split(" -> ") select (src: split[0], dest: split[1])).ToImmutableDictionary(s => s.src, s => (first: s.src.Substring(0, 1) + s.dest, second: s.dest + s.src.Substring(1, 1)));
var start = (
    from i in Range(0, recipe.Length - 1) let pair = recipe.Substring(i, 2) group pair by pair).ToImmutableDictionary(g => g.Key, g => (long)g.Count());
