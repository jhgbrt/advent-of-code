...
var part2 = (
    ...
    select size).Take(3).Aggregate(1, (a, i) => i * a);
...
class Grid
{
    ...
    public IEnumerable<Point> Neighbours(Point p) =>
        from d in new (int x, int y)[] { (-1, 0), (0, 1), (1, 0), (0, -1) } where (p.x + d.x, p.y + d.y) is ( >= 0 and < 100, >= 0 and < 100) select new Point(p.x + d.x, p.y + d.y);
}