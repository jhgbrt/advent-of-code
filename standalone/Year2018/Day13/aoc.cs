...
void Write(ImmutableDictionary<Coordinate, char> grid, ImmutableArray<Cart> carts)
{
    Console.Clear();
...
(ImmutableDictionary<Coordinate, char> grid, ImmutableArray<Cart> carts) BuildGrid()
{
    var grid = (
        from y in Enumerable.Range(0, input.Length)
        from x in Enumerable.Range(0, input[y].Length)
        where input[y][x] != ' '
        select (x, y, c: input[y][x])).ToImmutableDictionary(i => new Coordinate(i.x, i.y), i => i.c);
    var cartSymbols = new[] { '<', '>', 'v', '^' };
    int id = 0;
    var carts = (
        from kv in grid
        where cartSymbols.Contains(kv.Value)
        select new Cart(kv.Key, ++id, kv.Value, AtCrossing.TurnLeft)).OrderBy(c => c.Location.Y).ThenBy(c => c.Location.X).ToImmutableArray();
...
record Cart(Coordinate Location, int Id, char Symbol, AtCrossing AtNextCrossing)
{
    ...
    Cart Straight() => this with { Location = Next() };
