...
var part1 = input.Where(s => !string.IsNullOrEmpty(s)).Select(Parse.AsJsonElement).Chunked2().Select((pair, index) => (result: comparer.Compare(pair.a, pair.b), index: index + 1)).Where(item => item.result <= 0).Select(item => item.index).Sum();
var part2 = input.Where(s => !string.IsNullOrEmpty(s)).Select(Parse.AsJsonElement).Concat(dividers).ToImmutableSortedSet(comparer).Select((p, i) => (p, index: i + 1)).Join(dividers, outer => outer.p, p => p, (outer, _) => outer.index).Aggregate(1, (l, r) => l * r);
...
class MyJsonArrayComparer : IComparer<JsonElement>
{
    ...
    int Compare(JsonElement[] left, JsonElement[] right) => (
        ...
        where result != 0
        select result as int?).FirstOrDefault() ?? left.Length.CompareTo(right.Length);
