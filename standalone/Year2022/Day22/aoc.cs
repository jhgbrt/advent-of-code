...
long Part2()
{
    var (side, position, facing) = GetInstructions(instructions).Aggregate(cube.InitializeCrawler(), cube.Move);
    var (x, y) = side.origin;
...
record Cube(ImmutableDictionary<Side, SideData> sides, ImmutableArray<Delta> deltas, int length)
{
    ...
    {
        var neighbours = new[] { (side: Front, neighbours: new[] { Right, Bottom, Left, Top }.ToImmutableArray()), (side: Right, neighbours: new[] { Front, Top, Back, Bottom }.ToImmutableArray()), (side: Bottom, neighbours: new[] { Front, Right, Back, Left }.ToImmutableArray()), (side: Left, neighbours: new[] { Front, Bottom, Back, Top }.ToImmutableArray()), (side: Back, neighbours: new[] { Left, Bottom, Right, Top }.ToImmutableArray()), (side: Top, neighbours: new[] { Right, Front, Left, Back }.ToImmutableArray()) }.ToImmutableDictionary(x => x.side, x => x.neighbours);
        var tiles = (
