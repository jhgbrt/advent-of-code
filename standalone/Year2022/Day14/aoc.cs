...
record Line(Point start, Point end)
{
    ...
    public IEnumerable<Point> Points() => start.x == end.x ? Range(start.y, end.y - start.y + 1).Select(y => new Point(start.x, y)) : Range(start.x, end.x - start.x + 1).Select(x => new Point(x, start.y));
}
