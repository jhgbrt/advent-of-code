...
var blueprints = (
    ...
    select new Blueprint(id: int.Parse(match.Groups["id"].Value), ore: int.Parse(match.Groups["ore"].Value), clay: (ore: int.Parse(match.Groups["clay_ore"].Value), 0), obsidian: (ore: int.Parse(match.Groups["obsidian_ore"].Value), clay: int.Parse(match.Groups["obsidian_clay"].Value)), geode: (ore: int.Parse(match.Groups["geode_ore"].Value), obsidian: int.Parse(match.Groups["geode_obsidian"].Value)))).ToImmutableArray();
var sw = Stopwatch.StartNew();
var part1 = (
    from b in blueprints
    select b.id * UseBlueprint(b)).Sum();
var part2 = (
    from b in blueprints.Take(3) select UseBlueprint(b, 32)).Aggregate(1, (l, r) => l * r);
...
int UseBlueprint(Blueprint blueprint, int startTime = 24)
{
    ...
    while (queue.TryDequeue(out var item))
    {
        ...
        if (time == 0)
            continue;
        int nextTick = time - 1;
        state = state with { robots = state.robots with { ore = Math.Min(state.robots.ore, blueprint.MostExpensiveOre), clay = Math.Min(state.robots.clay, blueprint.obsidian.clay), obsidian = Math.Min(state.robots.obsidian, blueprint.geode.obsidian) }, resources = state.resources with { ore = Math.Min(state.resources.ore, (time * blueprint.MostExpensiveOre) - (state.robots.ore * nextTick)), clay = Math.Min(state.resources.clay, (time * blueprint.obsidian.clay) - (state.robots.clay * nextTick)), obsidian = Math.Min(state.resources.obsidian, (time * blueprint.geode.obsidian) - (state.robots.obsidian * nextTick)) } };
